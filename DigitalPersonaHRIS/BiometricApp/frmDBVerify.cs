                                ...
                                if (Convert.ToDouble(compare.Score.ToString()) == 0)
                                {
                                    ...
                                    using (MySqlDataReader reader = checkCmd.ExecuteReader())
                                    {
                                        if (reader.HasRows)
                                        {
                                            ...
                                            // Calculate working hours subtracting 1 hour
                                            int totalMinutes = (int)timeDifference.TotalMinutes;
                                            totalMinutes -= 60; //subtracting 1hour if has an error removed
                                            ...
                                            updateCmd.Parameters.AddWithValue("@workingHours", $"{hours} Hours {minutes} Minutes");
